...
function primary_admin() {
    let players = document.querySelector('section.primary .admin .players ol');
    let search = document.querySelector('section.primary .admin .players .search');
    let games = document.querySelector('section.primary .admin .events .games');
    let template = document.querySelector('section.primary .admin .events .template');

    search.addEventListener('keyup', function (event) {
        event.preventDefault();

        let items = players.querySelectorAll('li');
        if (search.value.length > 3) {
            console.log(1);
            for (let i = 0; i < items.length; i++) {
                if (!items[i].getAttribute('name').includes(search.value)) {
    ...
    function update_users() {
        ...
        connection.onmessage = function (e) {
            let users = e.data.split(' ');
            for (let i = 0; i < users.length; i++) {
                players.insertAdjacentHTML('beforeend', '<li class="item" name="' + users[i] + '"><img src="avatar/avatar (' + Math.floor(Math.random() * 50 + 1) + ').svg"><h2>' + users[i] + '</h2></li>')
            }
            connection.close();
        }
    }
    ...
    template.querySelector('button').addEventListener('click', function () {
        let message = template.querySelector('.message');
        let prize = template.querySelector('.info .prize');
        let sday, smonth, syear, fday, fmonth, fyear;
        sday = template.querySelector('.info .start .day');
        smonth = template.querySelector('.info .start .month');
        syear = template.querySelector('.info .start .year');
        fday = template.querySelector('.info .finish .day');
        fmonth = template.querySelector('.info .finish .month');
        fyear = template.querySelector('.info .finish .year');

        if (template.querySelector('.game').value.localeCompare('battle') == 0 || template.querySelector('.game').value.localeCompare('dots') == 0) {
            if (prize.value.length > 0 && parseInt(prize.value) > 0) {
                if (sday.value.length > 0 && parseInt(sday.value) > 0 && parseInt(sday.value) < 32 && smonth.value.length > 0 && parseInt(smonth.value) > 0 && parseInt(smonth.value) < 13 && syear.value.length > 0 && parseInt(syear.value) > 2020) {
                    if (fday.value.length > 0 && parseInt(fday.value) > 0 && parseInt(fday.value) < 32 && fmonth.value.length > 0 && parseInt(fmonth.value) > 0 && parseInt(fmonth.value) < 13 && fyear.value.length > 0 && parseInt(fyear.value) > 2020) {
                        ...
                        connection.onopen = function () {
                            connection.send('user addevent ' + username + " " + token + " " + template.querySelector('.game').value + " " + prize.value + ' ' + sday.value + ' ' + smonth.value + ' ' + syear.value + ' ' + fday.value + ' ' + fmonth.value + ' ' + fyear.value);
                        };
                        ...
                        connection.onmessage = function (e) {
                            ...
                            let date = new Date();
                            if (template.querySelector('.game').value.localeCompare('dots') == 0) {
